...
#FLetcher uses Gh1 instead of Gh
canonical <- data[c('Gh1', 'Prl','Cga','Tshb','Pomc','Pcsk2','Pax7','Crhr1'), ] 
...
ggplot(data=canonical.tr, aes(Pomc))+
  geom_histogram(binwidth = 0.1)
#create an object of which cells to keep based on canonical gene expression levels. 
Cort <- rownames(canonical.tr[canonical.tr$Pomc >4 & canonical.tr$Pomc <9 ,])
...
ggplot(data=canonical.tr, aes(Gh1))+
  geom_histogram(binwidth = 0.1)
#create an object of which cells to keep based on canonical gene expression levels. 
Somato <- rownames(canonical.tr[canonical.tr$Gh > 5,])
...
#Uncategorised 
numUncat <- length(colnames(data))
propUncat <- (numUncat/totalCells)*100

#write these to a csv
types <- c("Somatotrophs","Lactotrophs","Gonadotrophs","Thyrotrophs","Melanotrophs","Corticotrophs","Uncategorised")
proportions <- c(propSom, propLac,propGon,propThy,propMel,propCor,propUncat)
...
cortFromThreshold <- RunPCA(Corticotrophs,verbose= FALSE) 
...
cortFromThreshold <- subset(cortFromThreshold, idents =c("0","1","3") )  
...
saveRDS(cortFromThreshold, file = "../THresholdCorticotrophSubClusters.rds")
