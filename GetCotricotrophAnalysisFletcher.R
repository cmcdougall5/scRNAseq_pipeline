...
DimPlot(raw, label=TRUE) + NoLegend()+ ggtitle("Pituitary Clusters")
p<-DimPlot(raw, label=TRUE) + NoLegend()+ ggtitle("Pituitary Clusters")
...
saveRDS(cort, file = "CorticotrophSubClusters.rds")
...
#FOS is repeated in the DE genes which is messing with the plots
de.genes <- sort(unique(de.genes))
