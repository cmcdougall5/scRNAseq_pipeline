...
```
The raw count and gene data is read in from the 10X sequence data files described in [Ho et al](https://link.springer.com/article/10.1007/s13238-020-00705-x) and downloaded from [NCBI](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi).
...
These plots would suggest cluster 7 is the most likely corticotroph cluster.
...
``` {r corticoph differential expression, echo=FALSE, message=FALSE}
...
This analysis based on 17 of 30 possible principal components suggests cluster 7 to be the corticotrophs. Cluster 3 appears to be a cluster containing all cell types at a high level, double count cluster? This could be a result of the broadened second feature in the violin plot. 

Differential expression analysis showed the top differentially expressed gene found in 29% of corticodroph cells (and 1% of all other cells) was Khdrbs2.


When re-clustered, the corticotrophs demonstrate cell homogeneity with no visible sub clusters.
