...
subtitle: "Ho et al 8week male scRNA seq dataset"
...
This document details the analysis of the single RNA sequence data by [Ho et al](https://link.springer.com/article/10.1007/s13238-020-00705-x). available from [Ho et al](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi).
...
For the Mayran analysis, the Mayran folder is indicted in the path.
...
raw.data <- Read10X("Ho Data/Ho male")
...
The QC plots to inform which cells should be excluded on quality basis (low quality, double counts). In the case of the Mayran data,there are clearly two separate features in the feature data, so all cells below 20 counts are dropped and cells with >5% mitochondrial DNA are also dropped. Also drop features > 1900 counts, as there appears to be a second feature.
...
The elbow plot shows that the majority of the information is captured within the first 16 Principal components, so the last 10 are dropped during the next stage.
...
#UMAP dimensional reduction of the identified PCs, only first 20 PCs
data <- RunUMAP(data, dims=1:16, umap.method = "umap-learn", metric = "correlation", verbose=FALSE)
...
data <- FindNeighbors(data, dims=1:16, verbose=FALSE)
...
These plots would suggest cluster 5 (of 10) is the most likely corticotroph cluster. Cluster two is also a candidate, however this appears to be a mix of all cell types.

Try and examine cluster 5 on its own, to try and extract corticotrophs in a more robust manner.
...
new.cluster.ids <- c("0", "1", "2", "3", "4", "cort", "6","7","8","9","10")
...
Identify most of information retained in 15 PCs

``` {r dimension reduction on reclustyer, include = FALSE}
#dimensional reduction
#UMAP dimensional reduction of the identified PCs
cortico <- RunUMAP(cortico, dims=1:15, verbose=FALSE) #note because SCTransform passes 3000 features can run more PCs

#Find nearest neighbours
cortico <- FindNeighbors(cortico, dims=1:15, verbose=FALSE)
...
This analysis based on 16 of 30 possible principal components suggests cluster 5 to be the corticotophs. When re-clustered, the corticotrophs demonstrate cell homogeneity with no visible sub clusters.
