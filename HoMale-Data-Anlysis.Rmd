...
The QC plots to inform which cells should be excluded on quality basis (low quality, double counts). In the case of the Mayran data,there are clearly two separate features in the feature data, so all cells below 20 counts are dropped and cells with >5% mitochondrial DNA are also dropped. 

```{r Subset the data, include=FALSE } 
#Also drop features > 1900 counts, as there appears to be a second feature.# & nFeature_RNA < 1900
...
#UMAP dimensional reduction of the identified PCs, only first 20 PCs chnage dims!
...
These plots would suggest cluster 7 (of 10) is the most likely corticotroph cluster.
...
new.cluster.ids <- c("0", "1", "2", "3", "4", "5", "6","cort","8","9","10")
...
This analysis based on 17 of 30 possible principal components suggests cluster 7 to be the corticotophs. When re-clustered, the corticotrophs demonstrate cell homogeneity with no visible sub clusters.
