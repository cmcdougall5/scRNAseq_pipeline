...
```{r Loading in the Data & Initialising Seurat}
...
```{r Data visualisation/Quality control, fig.height=3.5}
...
``` {r Perform dimensional reduction (UMAP) }
...
``` {r exclude thyro, somato & lacto, fig.height=3.5}
...
``` {r exclude gonado, fig.height=3.5}
#Visualise the canonical marker genes 
...
``` {r exclude melano, fig.height=3.5}
#Visualise the canonical marker genes 
...
``` {r confirm corticotrophs, fig.height=3.5}
#Visualise the canonical marker genes 
...
``` {r corticoph differential expression}
...
``` {r dimension reduction on reclustyer}
...
```{r examin cluster differential expression} 
...
``` {r differential expression summary}
head(df)
...
This analysis based on 20 of 30 possible principal components identifies cluster 12 to be the corticotophs based on expression of canonical markers. Differential expression analysis showed the top differentially expressed gene found in 90% of corticodroh cells (and 16% of all other cells) was AW551984.
