...
```
Does it fit that the uncategorised cells and damaged/doublet clusters are not mixed in with the data, examine the PCas

``` {r does junk sit outside data}
 DimPlot(Ho.Seuratobj, reduction = "pca")
```
Can see that these clusters do indeed sit outside the bulk of the data, so happy that these can be ignored. Subset the data to remove these junk cells. Also remove the multi hormonal cells for clarity.
...
\newpage Examine the differential expression for the clusters
...
#want miniumu 2 fold differnce, set using logfc-threshold. log 2 is 0.3
markers <-FindAllMarkers(Ho.Seuratobj, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.6, )
