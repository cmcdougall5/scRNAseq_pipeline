...
This document details the analysis of the single RNA sequence data by [Mayran et al](https://www.nature.com/articles/s41467-019-11791-9). available from [Mayran et al](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi).
...
The QC plots to inform which cells should be excluded on quality basis (low quality, double counts). In the case of the Mayran data,there are clearly two sepearte features in the feature data, so all cells below 1000 counts are dropped and cells with >5% mitochondrial DNA are also dropped.

```{r Subset the data, include=FALSE }
data <- subset(data, subset = nFeature_RNA > 1000  & percent.mt < 5)
...
These plots would suggest cluster 6 (of 16) is the most likely corticotroph cluster.
...
This analysis based on 17 of 30 possible principal components shows cluster 6 to be the corticotophs. When re-clustered, the corticotrophs demonstrate cell heterogeneity with three visible sub clusters.A differential analysis of these three clusters shows the top differentiated genes in each cluster to be Jun & Tnnt1 in cluster zero, Pomc & Nmb in cluster one and Srxn1 & Mt2 in cluster three.  
